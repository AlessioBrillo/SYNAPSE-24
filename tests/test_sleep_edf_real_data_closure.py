...
# Canonical closure subjects: 2 healthy SC subjects, Fpz-Cz @100 Hz.
# Architect default (expert judgment): mean kappa gate over 2 subjects.
CLOSURE_SUBJECTS = ("SC4001", "SC4101")

requires_sc4001 = pytest.mark.skipif(
    not (SLEEP_EDF_DIR / "SC4001E0-PSG.edf").exists(),
    reason="Sleep-EDF SC4001 not cached",
)
requires_sc4101 = pytest.mark.skipif(
    not (SLEEP_EDF_DIR / "SC4101E0-PSG.edf").exists(),
    reason="Sleep-EDF SC4101 not cached",
)
...
class TestSleepEdfRealDataClosure:
    """YASA Fpz-Cz staging must clear the kappa gate on cached SC subjects."""
    ...
    def test_sc4001_fprz_cz_clears_per_subject_floor(self) -> None:
        ...
        assert float(result["kappa"]) >= 0.65
    ...
    @requires_sc4001
    @requires_sc4101
    def test_closure_mean_kappa_gate(self) -> None:
        """Phase 0 exit gate: mean Cohen's kappa >= 0.75 over closure subjects."""
        from synapse24.ingestion.sleep_edf import load_sleep_edf_record
        ...
        for subject in CLOSURE_SUBJECTS:
            ...
            kappas.append(float(_validate_fprz_cz(record)["kappa"]))
        assert float(np.mean(kappas)) >= 0.75
...
def _validate_fprz_cz(record: dict) -> dict:
    """Run YASA on the record's Fpz-Cz channel vs the gold hypnogram."""
    ...
    return validate_sleep_staging_against_gold(
        ...
        sampling_rate=int(fs_dict[channel]),
